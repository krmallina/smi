...
def get_stock_data(ticker, extended_hours=False):
    try:
        ...
        # Put/Call Volume Ratio & Implied Move
...
if __name__ == "__main__":
    ...
    # Map full path → just the filename for correct relative linking
    other_filename = {
        dashboards[0][1]: os.path.basename(dashboards[1][1]),
        dashboards[1][1]: os.path.basename(dashboards[0][1])
    }
    ...
    for ext, out, name, cls in dashboards:
        try:
            ...
            # Inject correct title and relative switch link
            with open(out, 'r+', encoding='utf-8') as f:
                ...
                switch = f'<a href="{other_filename[out]}" class="switch-btn">Switch to {"Extended" if not ext else "Regular"} Hours</a>'
    ...
    print("\nDashboards generated with correct cross-links.")